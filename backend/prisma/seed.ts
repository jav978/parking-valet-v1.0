import { PrismaClient, VehicleType, SpotStatus, RateType, DocumentType, ClientType } from '@prisma/client';
...
async function main() {
  ...
  // Supervisor: casi todo excepto gestión de usuarios/roles/permisos/backups
  ...
  // Cashier: tickets, payments, cash register, clients, vehicles
  ...
  const createdSpotTypes = await Promise.all(
    spotTypes.map((st) =>
      prisma.spotType.upsert({
        where: { code: st.code },
        create: st,
        update: { name: st.name },
      }),
  ...
  for (const floor of floors) {
    for (const section of sections) {
      for (let num = 1; num <= 5; num++) {
        ...
        const status: SpotStatus = SpotStatus.AVAILABLE;

        if (section === 'A' && num === 1) spotTypeId = handicapTypeId;
        else if (section === 'A' && num === 5) spotTypeId = evTypeId;
        else if (section === 'B' && num === 1) spotTypeId = vipTypeId;
  ...
  const client1 = await prisma.client.upsert({
    where: { documentType_documentNumber: { documentType: DocumentType.ID, documentNumber: 'CLI-001' } },
    create: {
      firstName: 'Juan',
  ...
  const client2 = await prisma.client.upsert({
    where: { documentType_documentNumber: { documentType: DocumentType.ID, documentNumber: 'CLI-002' } },
    create: {
      firstName: 'María',
  ...
  const client3 = await prisma.client.upsert({
    where: { documentType_documentNumber: { documentType: DocumentType.ID, documentNumber: 'CLI-003' } },
    create: {
      firstName: 'Carlos',
