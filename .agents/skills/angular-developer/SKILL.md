...
## Frontend UI Standards (PrimeNG)

When building Angular applications, always use PrimeNG as the primary UI component library.

### General Rules

1. Prefer PrimeNG components over native HTML elements whenever an equivalent component exists.
2. Use the latest stable version of PrimeNG compatible with the Angular version.
3. Use PrimeIcons for all icons.
4. Use PrimeFlex (or Tailwind CSS if the project uses Tailwind) for layouts and utility classes.
5. Keep the UI consistent by using PrimeNG themes and design tokens.
6. Avoid creating custom components when PrimeNG already provides one.
7. Follow PrimeNG accessibility recommendations.
...
Always integrate PrimeNG form controls with Angular Reactive Forms or Signal Forms depending on the Angular version.

Validation messages should use PrimeNG Message components.

### Tables

Use `p-table` as the standard data grid.
...
- Row Selection
- Context Menu
- CSV/Excel Export when required

### Dialogs

Use PrimeNG Dialog or DynamicDialog instead of custom modal implementations.
...
- Messages for inline validation.

Never use browser alerts unless explicitly requested.

### Icons

Always use PrimeIcons.

Avoid SVG icon packs unless required by the project.

### Styling

- Use PrimeNG themes.
- Customize using Design Tokens when possible.
- Avoid overriding PrimeNG CSS unless necessary.
- Use Tailwind CSS only for spacing, flexbox, grids and utilities.
...
- Use Angular Signals for state management when supported.

### Code Generation Rule

Whenever generating Angular code:

- Generate complete PrimeNG implementations.
- Import the necessary PrimeNG modules.
- Configure Reactive Forms or Signal Forms correctly.
- Produce responsive layouts using Tailwind + PrimeNG.
- Do not generate plain HTML controls if PrimeNG provides an equivalent component.
